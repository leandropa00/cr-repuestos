<?php
	...
	class Informe extends Tabla {
		...
		private $MostradorSoloFlotas = array();
		private $MostradorColision = array();
		private $mostrador_mantenimiento = array();
		private $MostadorOtrosVentasExternas = array();
		private $MostradorSolochevrolet = array();
		
		private $MecanicaRapidaFlotas = array();
		private $MecanicaRapidaUno = array();
		private $MecanicaEspecializadaFlotas = array();
		private $MecanicaEspecializadaUno = array();

		private $ColisionUno = array();
		private $ColisionAseguradoras = array();
		private $TotalGarantias = array();
		private $AlternosTaller = array();
		private $AlternosColision = array();
		private $AlternosMostrador = array();

		private $VentasMostrador = array();
		private $VentasTaller = array();
		private $VentasColision = array();
		private $VentasGarantias = array();
		private $VentasInternos = array();
		private $VentasAlternos = array();
		private $TotalComprasRepuestos = array();
		
		private $InventariosEntregadoAServicio = array();
		private $getTotalFOF = array();
		...
		*/
		...
		public function getAlternosColision($tipo_vehiculo = 'liviano') {
			if (isset($this->AlternosColision[$this->id][$tipo_vehiculo])) return $this->AlternosColision[$this->id][$tipo_vehiculo];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.total) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
					...
					AND v.nombre_grupo NOT IN ('ACCES')
					AND vehiculo_tipo='$tipo_vehiculo'");
			$this->AlternosColision[$this->id][$tipo_vehiculo] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			...
			$r = self::queryMYSQL("select sum(v.total) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->AlternosColision[$this->id][$tipo_vehiculo] -= $devoluciones;
			return $this->AlternosColision[$this->id][$tipo_vehiculo];
		}
		...
		public function getAlternosMostrador($tipo_vehiculo = 'liviano') {
			if (isset($this->AlternosMostrador[$this->id][$tipo_vehiculo])) return $this->AlternosMostrador[$this->id][$tipo_vehiculo];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.total) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
					...
					AND v.nombre_grupo NOT IN ('ACCES')
					AND vehiculo_tipo='$tipo_vehiculo'");
			$this->AlternosMostrador[$this->id][$tipo_vehiculo] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			...
			$r = self::queryMYSQL("select sum(v.total) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->AlternosMostrador[$this->id][$tipo_vehiculo] -= $devoluciones;
			return $this->AlternosMostrador[$this->id][$tipo_vehiculo];
		}
		...
		public function getVentasMostrador() {	//TODO: parámetro
			if (isset($this->VentasMostrador[$this->id])) return $this->VentasMostrador[$this->id];
			BD::changeInstancia("mysql");
			...
			$this->VentasMostrador[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			$codigo = 201;
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->VentasMostrador[$this->id] -= $devoluciones;
			return $this->VentasMostrador[$this->id];
		}

		public function getVentasMostradorData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getVentasTaller() {
			if (isset($this->VentasTaller[$this->id])) return $this->VentasTaller[$this->id];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.totalc) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
					...
					AND v.devolucion=0 AND v.sw in (1,11) AND v.nombre_grupo NOT IN ('ACCES')");
			$this->VentasTaller[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			$codigo = 202;
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->VentasTaller[$this->id] -= $devoluciones;
			return $this->VentasTaller[$this->id];
		}

		public function getVentasTallerData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getVentasColision() {
			if (isset($this->VentasColision[$this->id])) return $this->VentasColision[$this->id];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.totalc) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
					...
					AND v.tipo in ('FL')
					AND v.devolucion=0 AND v.sw=1 AND v.nombre_grupo NOT IN ('ACCES')");
			$this->VentasColision[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			$codigo = 203;
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->VentasColision[$this->id] -= $devoluciones;
			return $this->VentasColision[$this->id];
		}

		public function getVentasColisionData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getVentasGarantias() {
			if (isset($this->VentasGarantias[$this->id])) return $this->VentasGarantias[$this->id];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.totalc) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
					...
					AND v.tipo in ('FG')
					AND v.devolucion=0 AND v.sw=1 AND v.nombre_grupo NOT IN ('ACCES')");
			$this->VentasGarantias[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			$codigo = 204;
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->VentasGarantias[$this->id] -= $devoluciones;
			return $this->VentasGarantias[$this->id];
		}

		public function getVentasGarantiasData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getVentasInternos() {
			if (isset($this->VentasInternos[$this->id])) return $this->VentasInternos[$this->id];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.totalc) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
					...
					AND v.tipo in ('TI')
					AND v.devolucion=0 AND v.sw=1 AND v.nombre_grupo NOT IN ('ACCES')");
			$this->VentasInternos[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			$codigo = 205;
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->VentasInternos[$this->id] -= $devoluciones;
			return $this->VentasInternos[$this->id];
		}

		public function getVentasInternosData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getVentasAlternos() {
			if (isset($this->VentasAlternos[$this->id])) return $this->VentasAlternos[$this->id];
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.totalc) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
			...
			$this->VentasAlternos[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			//Devoluciones de meses anteriores
			$codigo = 206;
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo=$codigo AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$this->VentasAlternos[$this->id] -= $devoluciones;
			return $this->VentasAlternos[$this->id];
		}

		public function getVentasAlternosData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getTotalComprasRepuestos($tipodocs = array()) {
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select sum(v.totalc) total 
				FROM rp_ventasxasesor v 
				WHERE v.informe_id=" . $this->id . " 
			...
			$r = self::queryMYSQL("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo in (" . implode(",", $codigos) . ") AND v.informe_id<>" . $this->id . " and concat(v.tipo, v.numero) in (
					select concat(d.tipo_link,d.numero_link) from rp_ventasxasesor d where d.sw=2 and d.informe_id=" . $this->id . ")");
			$devoluciones = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			$total -= $devoluciones;
			return $total;
		...
		public function getTotalComprasRepuestosData($tipodocs = array()) {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select v.*, c.nombre cliente_nombre
				FROM rp_ventasxasesor v 
		...
		public function getTotalInventarios() {
			return $this->getInventariosEntregadoAServicio()
				+ $this->getInventarios('0M-12M')
				+ $this->getInventarios('12M-24M')
				+ $this->getInventarios('24M-MAS')
				+ $this->getInventariosAlternos();
		}

		public function getInventariosEntregadoAServicio() {
			if (isset($this->InventariosEntregadoAServicio[$this->id])) return $this->InventariosEntregadoAServicio[$this->id];
			BD::changeInstancia("mysql");
			...
			$this->InventariosEntregadoAServicio[$this->id] = ($f = BD::obtenerRegistro($r)) ? $f["total"] : 0;
			return $this->InventariosEntregadoAServicio[$this->id];
		}

		public function getInventariosEntregadoAServicioData() {
			$result = array();
			BD::changeInstancia("mysql");
		...
		public function getInventarios($edad) {
			BD::changeInstancia("mysql");
			$query = "select sum(costo_promedio * stock) total
		...
		public function getInventariosData($edad) {
			BD::changeInstancia("mysql");
			$result = array();
		...
		public function getInventariosAlternos() {
			BD::changeInstancia("mysql");
			$query = "select sum(costo_promedio * stock) total
				from rp_ubicacion_repuestos 
				where informe_id=" . $this->id . " 
		...
		public function getInventariosAlternosData() {
			$result = array();
			BD::changeInstancia("mysql");
			$r = self::queryMYSQL("select *, costo_promedio * stock xtotal
				from rp_ubicacion_repuestos where informe_id=" . $this->id . " 
		...
		public function getTotalFOF() {
			if (isset($this->getTotalFOF[$this->id])) return $this->getTotalFOF[$this->id];
			BD::changeInstancia("mysql");
			$query = "select avg(fof) total from rp_perfil_taller where informe_id=" . $this->id;
			$r = self::queryMYSQL($query) or die("Error en query: $query");
			$this->getTotalFOF[$this->id] = ($f = BD::obtenerRegistro($r)) ? round($f["total"], 2) : 0;
			return $this->getTotalFOF[$this->id];
		}
		...
		public function actualizarDatos() {
			...
			$r = self::queryMYSQL("SELECT tipo, numero, fec, codigo, cantidad, valor_unitario, nit, tipo_link, numero_link
				FROM documentos_lin
			...
			$r = self::queryMYSQL("select * from ubicacion_repuestos where ano=$xxanio and mes=$xxmes") or die("Error query");
			while ($f = BD::obtenerRegistro($r)) {
			...
			BD::changeInstancia("facts");
			$insertados = 0;
			$r = self::queryMYSQL("select * from perfil_cliente_comandos where tipo=1 and instruccion is not null order by id asc") or die("Error query");
			while ($f = BD::obtenerRegistro($r)) {
				$query = $f["instruccion"];
				if (strpos($query, "@mes") === FALSE) {
					...
					$rx = self::queryMYSQL($query) or die("Error en query $query");
			...
			$r = self::queryMYSQL($query) or die("Error query $query");
			while ($f = BD::obtenerRegistro($r)) {
				//guardamos en mysql
				$f["informe_id"] = $this->id;
				$tipo_proveedor = "gm";
