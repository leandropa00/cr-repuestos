...
class dbal_mysql extends dbal
{
	...
	*/
	function sql_connect($sqlserver, $sqluser, $sqlpassword, $database, $port = false, $persistency = false, $new_link = false)
	{
		...
		$this->db_connect_id = ($this->persistency) ? mysqli_pconnect($this->server, $this->user, $sqlpassword, $new_link) : mysqli_connect($this->server, $this->user, $sqlpassword);
	...
	* @access	public
	*/
	function sql_query($query = '', $cache_ttl = 0, $debug_mode = false)
	{
		if ($query != '')
		{
			global $cache;
			...
			if ($this->query_result === false)
			{
				...
				{
					if ($debug_mode)
						echo mysqli_error($this->db_connect_id);
					return FALSE;
//					$this->sql_error($query);
	...
	*/
	function sql_escape($msg)
	{
		...
		return @mysqli_real_escape_string($msg, $this->db_connect_id);
	}
