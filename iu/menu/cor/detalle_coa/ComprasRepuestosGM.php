<?php
    ...
    $tipo_vehiculo = isset($_GET["tipov"]) ? $_GET["tipov"] : "";
...
<div style='overflow-y: scroll; max-height: 400px;'>
    <table class="table table-condensed table-hover table-bordered" style='font-size:12px;'>
		...
		<?php
            ...
            $total_SD = $total_CD = 0;
			foreach($informe->getTotalComprasRepuestosData($tipos, $tipo_vehiculo, true) as $data) {
			...
			$r = BD::sql_query("select sum(v.totalc) total FROM rp_ventasxasesor v 
				WHERE v.ubicacion_item_costo in (" . implode(",", $codigos) . ") AND v.nombre_grupo in ('ACCES') AND v.informe_id<>" . $informe->id . " $query_add and concat(v.tipo, v.numero) in (
