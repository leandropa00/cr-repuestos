...
<div style='overflow-y: scroll; max-height: 400px;'>
    <table class="table table-condensed table-hover table-bordered" style='font-size:12px;'>
		...
		<?php
            ...
            $total_SD = $total_CD = 0;
			foreach($informe->getRepuestosFlotasOtrasMarcasData($tipo_vehiculo, true) as $data) {
		...
		?>
            ...
            <?php
                $r = BD::sql_query("select v.*, c.nombre cliente_nombre 
                    ...
                    WHERE v.ubicacion_item in ($codigo)  AND v.nombre_grupo in ('ACCES') 
                        AND replace(v.referencia, '*/', '') not in (select material from rp_maestro)
                        AND v.informe_id<>" . $informe->id . " and concat(v.tipo, v.numero) in (
